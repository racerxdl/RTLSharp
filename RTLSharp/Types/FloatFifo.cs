...
namespace RTLSharp.Types {
  ...
  /// Float FIFO
  /// </summary>
  public class FloatFifo : IDisposable {
    #region Constants
    private const int BlockSize = 65536 / sizeof(float);
    ...
    public unsafe void Write(float* buffer, int bufOffset, int length) {
      lock (this) {
        int bytesLeft = length;
        while (bytesLeft > 0) {
          int toWrite = Math.Min(BlockSize - _writePos, bytesLeft);
          DataBuffer block = GetBlock();
          float* blockPtr = (float*)block;
          Utils.memcpy(blockPtr + _writePos, buffer + bufOffset + length - bytesLeft, toWrite * sizeof(float));
          _writePos += toWrite;
          bytesLeft -= toWrite;
        }
        _size += length;
    ...
    public unsafe int Read(float* buf, int count) => Read(buf, 0, count);
    public unsafe void Write(float* buf, int count) => Write(buf, 0, count);
    #endregion
    #region Properties
    public int Length {
      get { return _size; }
    ...
    private unsafe DataBuffer AllocBlock() {
      var result = _stackCache.Count > 0 ? _stackCache.Pop() : DataBuffer.Create(BlockSize, sizeof(float));
      return result;
    }
    ...
    private unsafe int ReadData(float* buf, int ofs, int count) {
      var sizeLeft = count;
